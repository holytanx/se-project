...
<v-dialog v-model="dialog_edit_sub" persistent max-width="600px">
  ...
  <v-dialog v-model="dialog_addsubproject" persistent max-width="600px">
      <template v-slot:activator="{ on }">
          ...
          <v-btn color="success" class="col-auto" dark v-on="on">เพิ่มโครงการย่อย</v-btn>
  ...
  <v-dialog v-model="dialog_expense" persistent max-width="600px">
      ...
      <v-card>
        ...
        <v-card-text>
          <v-container>
            <v-row>
              ...
              <v-col cols="12">
                <v-text-field v-model="expense_amount" label="จำนวนเงิน (บาท)" type="number" ></v-text-field>
              </v-col>
                    
              <v-col cols="12">
                <v-text-field v-model="expense_description" label="คำอธิบาย" type="text" ></v-text-field>
              </v-col>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="danger" text @click="dialog_expense = false">ปิด</v-btn>
          <v-btn color="success" text @click="expense_subproject">เพิ่ม</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>          
          </v-col>
  ...
  <v-dialog v-model="dialog_disburse" persistent max-width="600px">
      ...
      <v-card>
        ...
        <v-card-text>
          <v-container>
            <v-row>
              ...
              <v-col cols="12">
                <v-text-field v-model="disburse_amount" label="จำนวนเงิน (บาท)" type="number" ></v-text-field>
              </v-col>
              <v-col cols="12">
                <v-text-field v-model="disburse_description" label="คำอธิบาย" type="text" ></v-text-field>
              </v-col>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="danger" text @click="dialog_disburse = false">ปิด</v-btn>
          <v-btn color="success" text @click="disburse_subproject">เพิ่ม</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>          
          </v-col>
...
export default {
  ...
  methods: {
      ...
      expense_subproject(){
        ...
        db.collection(this.sub_project_doc).doc(this.expense_sub_project_id[0]).update({
          expense_remain: firebase.firestore.FieldValue.increment(-parseInt(this.expense_amount)),
          expense_amount: firebase.firestore.FieldValue.increment(parseInt(this.expense_amount)),
        })
      ...
      disburse_subproject(){
        ...
        db.collection(this.sub_project_doc).doc(this.disburse_sub_project_id[0]).update({
          disburse_remain: firebase.firestore.FieldValue.increment(-parseInt(this.disburse_amount)),
          disburse_amount: firebase.firestore.FieldValue.increment(parseInt(this.disburse_amount)),
        })
