<template>
  ...
  <v-dialog v-model="dialog_add_si" persistent max-width="600px">
      <template v-slot:activator="{ on }">
        <v-btn color="success" dark v-on="on">เพิ่มประเด็นยุทธ์ศาสตร์
      ...
      <v-card>
        ...
        <v-card-text>
          <v-container>
            <v-row>     
            <v-col cols="12">
                <v-text-field label="ประเด็นยุทธ์ศาสตร์ที่*" v-model="si_id" type="number" required></v-text-field>
              </v-col>                   
            <v-col cols="12">
                <v-text-field label="ชื่อ(รายละเอียด)*" v-mode="si_name" type="name" required></v-text-field>
              </v-col>     
            </v-row>
          </v-container>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="error" text @click="dialog_add_si = false">ยกเลิก</v-btn>
...
<v-dialog v-model="dialog_edit_si" persistent max-width="600px">
      <v-card>
        ...
        <v-card-text>
          <v-container>
            <v-row>
              ...
              <v-col cols="12">
                <v-text-field v-model="editedItem_si.si_name" label="ชื่อ(รายละเอียด)" required></v-text-field>
              </v-col>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="error" text @click="dialog_edit_si = false">ยกเลิก</v-btn>
  ...
  <v-dialog v-model="dialog_add_s" persistent max-width="600px">
      <template v-slot:activator="{ on }">
        <v-btn color="warning" dark v-on="on">เพิ่มยุทธ์ศาสตร์
        <v-icon right dark>mdi-cloud-upload</v-icon>
        </v-btn>
      </template>
      <v-card>
        ...
        <v-card-text>
          <v-container>
            <v-row>     
            <v-col cols="12">
                <v-text-field label="ยุทธ์ศาสตร์ที่*" v-model="s_id" type="number" required></v-text-field>
              </v-col>                   
            <v-col cols="12">
                <v-text-field label="ชื่อ(รายละเอียด)*" v-mode="s_name" type="name" required></v-text-field>
              </v-col>     
            </v-row>
          </v-container>
          <small>*indicates required field</small>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="error" text @click="dialog_add_s = false">ยกเลิก</v-btn>
    ...
    <v-dialog v-model="dialog_edit_s" persistent max-width="600px">
      <v-card>
        ...
        <v-card-text>
          <v-container>
            <v-row>
              ...
              <v-col cols="12">
                <v-text-field v-model="editedItem_s.s_name" label="ชื่อ(รายละเอียด)" required></v-text-field>
              </v-col>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="error" text @click="dialog_edit_s = false">ยกเลิก</v-btn>
  ...
  <v-dialog v-model="dialog_add_t" persistent max-width="600px">
      <template v-slot:activator="{ on }">
        <v-btn color="error" dark v-on="on">เพิ่มกลยุทธ์
        <v-icon right dark>mdi-cloud-upload</v-icon>
        </v-btn>
      </template>
      <v-card>
        ...
        <v-card-text>
          <v-container>
            <v-row>     
            <v-col cols="12">
                <v-text-field label="กลยุทธ์ที่*" v-model="t_id" type="number" required></v-text-field>
              </v-col>                   
            <v-col cols="12">
                <v-text-field label="ชื่อ(รายละเอียด)*" v-mode="t_name" type="name" required></v-text-field>
              </v-col>     
            </v-row>
          </v-container>
          <small>*indicates required field</small>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="error" text @click="dialog_add_t = false">ยกเลิก</v-btn>
    ...
    <v-dialog v-model="dialog_edit_t" persistent max-width="600px">
      <v-card>
        ...
        <v-card-text>
          <v-container>
            <v-row>
              ...
              <v-col cols="12">
                <v-text-field v-model="editedItem_t.t_name" label="ชื่อ(รายละเอียด)" required></v-text-field>
              </v-col>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="error" text @click="dialog_edit_t = false">ยกเลิก</v-btn>
  ...
  >
    ...
    <template v-slot:item.si_id="{ item }">
      <v-chip :color="getColor_si(item.si_id)" dark>{{ item.si_id }}</v-chip>
    </template>
  ...
  >
    ...
    <template v-slot:item.s_id="{ item }">
      <v-chip :color="getColor_s(item.s_id)" dark>{{ item.s_id }}</v-chip>
    </template>
            <template v-slot:item.action="{ item }">
       <v-icon
        small
  ...
  >
    ...
    <template v-slot:item.t_id="{ item }">
      <v-chip :color="getColor_t(item.t_id)" dark>{{ item.t_id }}</v-chip>
    </template>
        <template v-slot:item.action="{ item }">
       <v-icon
        small
...
// import {addAdminRole} from '../admin.js'
  export default {
    ...
    methods: {
     ...
     db.collection('tactics').add({
      ...
      save_edit_si(){
        ...
        db.collection('strategicissues').doc(item.id).update({
      ...
      save_edit_s(){
        ...
        db.collection('strategic').doc(item.id).update({
            's_name': item.s_name
        })
      ...
      save_edit_t(){
        ...
        db.collection('tactics').doc(item.id).update({
            't_name': item.t_name
        })
    ...
    created(){
     ...
     db.collection('strategicissues').orderBy('si_id','asc').get().then(
      querySnapshot => {
     ...
     db.collection('strategic').orderBy('s_id','asc').get().then(
      querySnapshot => {
        querySnapshot.forEach(doc => {
          const data ={
            ...doc.data(),
     ...
     db.collection('tactics').orderBy('t_id','asc').get().then(
      querySnapshot => {
        querySnapshot.forEach(doc => {
          const data ={
            ...doc.data(),
